...
func (s *AntigravityGatewayService) TestConnection(ctx context.Context, account *Account, modelID string, customPrompt ...string) (*TestConnectionResult, error) {
	...
	if strings.HasPrefix(modelID, "gemini-") {
		requestBody, err = s.buildGeminiTestRequest(projectID, mappedModel, customPrompt...)
	} else {
		requestBody, err = s.buildClaudeTestRequest(projectID, mappedModel, customPrompt...)
	}
	if err != nil {
...
func (s *AntigravityGatewayService) buildGeminiTestRequest(projectID, model string, customPrompt ...string) ([]byte, error) {
	prompt, maxTokens := ".", 1
	if len(customPrompt) > 0 && strings.TrimSpace(customPrompt[0]) != "" {
		prompt, maxTokens = customPrompt[0], 4096
	}
...
func (s *AntigravityGatewayService) buildClaudeTestRequest(projectID, mappedModel string, customPrompt ...string) ([]byte, error) {
	prompt, maxTokens := ".", 1
	if len(customPrompt) > 0 && strings.TrimSpace(customPrompt[0]) != "" {
		prompt, maxTokens = customPrompt[0], 4096
	}
