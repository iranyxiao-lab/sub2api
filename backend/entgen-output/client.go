...
type Client struct {
	...
	// ChainScanCursor is the client for interacting with the ChainScanCursor builders.
	ChainScanCursor *ChainScanCursorClient
	...
	// EthereumGasFunding is the client for interacting with the EthereumGasFunding builders.
	EthereumGasFunding *EthereumGasFundingClient
	// EthereumNonceState is the client for interacting with the EthereumNonceState builders.
	EthereumNonceState *EthereumNonceStateClient
	...
	// OnchainPaymentIntent is the client for interacting with the OnchainPaymentIntent builders.
	OnchainPaymentIntent *OnchainPaymentIntentClient
	...
	// WalletDerivationCursor is the client for interacting with the WalletDerivationCursor builders.
	WalletDerivationCursor *WalletDerivationCursorClient
	// WalletSweep is the client for interacting with the WalletSweep builders.
	WalletSweep *WalletSweepClient
}
...
func (c *Client) init() {
	...
	c.ChainScanCursor = NewChainScanCursorClient(c.config)
	...
	c.EthereumGasFunding = NewEthereumGasFundingClient(c.config)
	c.EthereumNonceState = NewEthereumNonceStateClient(c.config)
	c.Group = NewGroupClient(c.config)
	c.IdempotencyRecord = NewIdempotencyRecordClient(c.config)
	c.IdentityAdoptionDecision = NewIdentityAdoptionDecisionClient(c.config)
	c.OnchainDeposit = NewOnchainDepositClient(c.config)
	c.OnchainPaymentIntent = NewOnchainPaymentIntentClient(c.config)
	...
	c.WalletDerivationCursor = NewWalletDerivationCursorClient(c.config)
	c.WalletSweep = NewWalletSweepClient(c.config)
}
...
func (c *Client) Tx(ctx context.Context) (*Tx, error) {
	...
	return &Tx{
		...
		ChainScanCursor:               NewChainScanCursorClient(cfg),
		...
		EthereumGasFunding:            NewEthereumGasFundingClient(cfg),
		EthereumNonceState:            NewEthereumNonceStateClient(cfg),
		Group:                         NewGroupClient(cfg),
		IdempotencyRecord:             NewIdempotencyRecordClient(cfg),
		IdentityAdoptionDecision:      NewIdentityAdoptionDecisionClient(cfg),
		OnchainDeposit:                NewOnchainDepositClient(cfg),
		OnchainPaymentIntent:          NewOnchainPaymentIntentClient(cfg),
		...
		WalletDerivationCursor:        NewWalletDerivationCursorClient(cfg),
		WalletSweep:                   NewWalletSweepClient(cfg),
	}, nil
}
...
func (c *Client) Use(hooks ...Hook) {
	for _, n := range []interface{ Use(...Hook) }{
		...
		c.BatchImageJob, c.ChainScanCursor, c.ChannelMonitor,
		c.ChannelMonitorDailyRollup, c.ChannelMonitorHistory,
		c.ChannelMonitorRequestTemplate, c.CompositeModelRoute, c.ErrorPassthroughRule,
		c.EthereumGasFunding, c.EthereumNonceState, c.Group, c.IdempotencyRecord,
		c.IdentityAdoptionDecision, c.OnchainDeposit, c.OnchainPaymentIntent,
		c.PaymentAuditLog, c.PaymentOrder, c.PaymentProviderInstance,
		c.PendingAuthSession, c.PromoCode, c.PromoCodeUsage, c.Proxy, c.RedeemCode,
		c.SecuritySecret, c.Setting, c.SubscriptionPlan, c.TLSFingerprintProfile,
		c.UsageCleanupTask, c.UsageLog, c.User, c.UserAllowedGroup,
		c.UserAttributeDefinition, c.UserAttributeValue, c.UserPlatformQuota,
		c.UserSubscription, c.WalletDerivationCursor, c.WalletSweep,
	} {
		n.Use(hooks...)
	}
}
...
func (c *Client) Mutate(ctx context.Context, m Mutation) (Value, error) {
	...
	case *ChainScanCursorMutation:
		return c.ChainScanCursor.mutate(ctx, m)
	...
	case *EthereumGasFundingMutation:
		return c.EthereumGasFunding.mutate(ctx, m)
	case *EthereumNonceStateMutation:
		return c.EthereumNonceState.mutate(ctx, m)
	case *GroupMutation:
	...
	case *OnchainPaymentIntentMutation:
		return c.OnchainPaymentIntent.mutate(ctx, m)
	...
	case *WalletDerivationCursorMutation:
		return c.WalletDerivationCursor.mutate(ctx, m)
	case *WalletSweepMutation:
		return c.WalletSweep.mutate(ctx, m)
	default:
...
// ChainScanCursorClient is a client for the ChainScanCursor schema.
type ChainScanCursorClient struct {
	config
}

// NewChainScanCursorClient returns a client for the ChainScanCursor from the given config.
func NewChainScanCursorClient(c config) *ChainScanCursorClient {
	return &ChainScanCursorClient{config: c}
}

// Use adds a list of mutation hooks to the hooks stack.
// A call to `Use(f, g, h)` equals to `chainscancursor.Hooks(f(g(h())))`.
func (c *ChainScanCursorClient) Use(hooks ...Hook) {
	c.hooks.ChainScanCursor = append(c.hooks.ChainScanCursor, hooks...)
}

// Intercept adds a list of query interceptors to the interceptors stack.
// A call to `Intercept(f, g, h)` equals to `chainscancursor.Intercept(f(g(h())))`.
func (c *ChainScanCursorClient) Intercept(interceptors ...Interceptor) {
	c.inters.ChainScanCursor = append(c.inters.ChainScanCursor, interceptors...)
}

// Create returns a builder for creating a ChainScanCursor entity.
func (c *ChainScanCursorClient) Create() *ChainScanCursorCreate {
	mutation := newChainScanCursorMutation(c.config, OpCreate)
	return &ChainScanCursorCreate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// CreateBulk returns a builder for creating a bulk of ChainScanCursor entities.
func (c *ChainScanCursorClient) CreateBulk(builders ...*ChainScanCursorCreate) *ChainScanCursorCreateBulk {
	return &ChainScanCursorCreateBulk{config: c.config, builders: builders}
}

// MapCreateBulk creates a bulk creation builder from the given slice. For each item in the slice, the function creates
// a builder and applies setFunc on it.
func (c *ChainScanCursorClient) MapCreateBulk(slice any, setFunc func(*ChainScanCursorCreate, int)) *ChainScanCursorCreateBulk {
	rv := reflect.ValueOf(slice)
	if rv.Kind() != reflect.Slice {
		return &ChainScanCursorCreateBulk{err: fmt.Errorf("calling to ChainScanCursorClient.MapCreateBulk with wrong type %T, need slice", slice)}
	}
	builders := make([]*ChainScanCursorCreate, rv.Len())
	for i := 0; i < rv.Len(); i++ {
		builders[i] = c.Create()
		setFunc(builders[i], i)
	}
...
// Update returns an update builder for ChainScanCursor.
func (c *ChainScanCursorClient) Update() *ChainScanCursorUpdate {
	mutation := newChainScanCursorMutation(c.config, OpUpdate)
	return &ChainScanCursorUpdate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOne returns an update builder for the given entity.
func (c *ChainScanCursorClient) UpdateOne(_m *ChainScanCursor) *ChainScanCursorUpdateOne {
	mutation := newChainScanCursorMutation(c.config, OpUpdateOne, withChainScanCursor(_m))
	return &ChainScanCursorUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOneID returns an update builder for the given id.
func (c *ChainScanCursorClient) UpdateOneID(id int64) *ChainScanCursorUpdateOne {
	mutation := newChainScanCursorMutation(c.config, OpUpdateOne, withChainScanCursorID(id))
	return &ChainScanCursorUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// Delete returns a delete builder for ChainScanCursor.
func (c *ChainScanCursorClient) Delete() *ChainScanCursorDelete {
	mutation := newChainScanCursorMutation(c.config, OpDelete)
	return &ChainScanCursorDelete{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// DeleteOne returns a builder for deleting the given entity.
func (c *ChainScanCursorClient) DeleteOne(_m *ChainScanCursor) *ChainScanCursorDeleteOne {
	return c.DeleteOneID(_m.ID)
}

// DeleteOneID returns a builder for deleting the given entity by its id.
func (c *ChainScanCursorClient) DeleteOneID(id int64) *ChainScanCursorDeleteOne {
	builder := c.Delete().Where(chainscancursor.ID(id))
	builder.mutation.id = &id
	builder.mutation.op = OpDeleteOne
	return &ChainScanCursorDeleteOne{builder}
}

// Query returns a query builder for ChainScanCursor.
func (c *ChainScanCursorClient) Query() *ChainScanCursorQuery {
	return &ChainScanCursorQuery{
		config: c.config,
		ctx:    &QueryContext{Type: TypeChainScanCursor},
		inters: c.Interceptors(),
	}
}

// Get returns a ChainScanCursor entity by its id.
func (c *ChainScanCursorClient) Get(ctx context.Context, id int64) (*ChainScanCursor, error) {
	return c.Query().Where(chainscancursor.ID(id)).Only(ctx)
}

// GetX is like Get, but panics if an error occurs.
func (c *ChainScanCursorClient) GetX(ctx context.Context, id int64) *ChainScanCursor {
	obj, err := c.Get(ctx, id)
	if err != nil {
		panic(err)
	}
...
// Hooks returns the client hooks.
func (c *ChainScanCursorClient) Hooks() []Hook {
	return c.hooks.ChainScanCursor
}

// Interceptors returns the client interceptors.
func (c *ChainScanCursorClient) Interceptors() []Interceptor {
	return c.inters.ChainScanCursor
}

func (c *ChainScanCursorClient) mutate(ctx context.Context, m *ChainScanCursorMutation) (Value, error) {
	switch m.Op() {
	case OpCreate:
		return (&ChainScanCursorCreate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdate:
		return (&ChainScanCursorUpdate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdateOne:
		return (&ChainScanCursorUpdateOne{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpDelete, OpDeleteOne:
		return (&ChainScanCursorDelete{config: c.config, hooks: c.Hooks(), mutation: m}).Exec(ctx)
	default:
		return nil, fmt.Errorf("ent: unknown ChainScanCursor mutation op: %q", m.Op())
	}
}
...
// EthereumGasFundingClient is a client for the EthereumGasFunding schema.
type EthereumGasFundingClient struct {
	config
}

// NewEthereumGasFundingClient returns a client for the EthereumGasFunding from the given config.
func NewEthereumGasFundingClient(c config) *EthereumGasFundingClient {
	return &EthereumGasFundingClient{config: c}
}

// Use adds a list of mutation hooks to the hooks stack.
// A call to `Use(f, g, h)` equals to `ethereumgasfunding.Hooks(f(g(h())))`.
func (c *EthereumGasFundingClient) Use(hooks ...Hook) {
	c.hooks.EthereumGasFunding = append(c.hooks.EthereumGasFunding, hooks...)
}

// Intercept adds a list of query interceptors to the interceptors stack.
// A call to `Intercept(f, g, h)` equals to `ethereumgasfunding.Intercept(f(g(h())))`.
func (c *EthereumGasFundingClient) Intercept(interceptors ...Interceptor) {
	c.inters.EthereumGasFunding = append(c.inters.EthereumGasFunding, interceptors...)
}

// Create returns a builder for creating a EthereumGasFunding entity.
func (c *EthereumGasFundingClient) Create() *EthereumGasFundingCreate {
	mutation := newEthereumGasFundingMutation(c.config, OpCreate)
	return &EthereumGasFundingCreate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// CreateBulk returns a builder for creating a bulk of EthereumGasFunding entities.
func (c *EthereumGasFundingClient) CreateBulk(builders ...*EthereumGasFundingCreate) *EthereumGasFundingCreateBulk {
	return &EthereumGasFundingCreateBulk{config: c.config, builders: builders}
}

// MapCreateBulk creates a bulk creation builder from the given slice. For each item in the slice, the function creates
// a builder and applies setFunc on it.
func (c *EthereumGasFundingClient) MapCreateBulk(slice any, setFunc func(*EthereumGasFundingCreate, int)) *EthereumGasFundingCreateBulk {
	rv := reflect.ValueOf(slice)
	if rv.Kind() != reflect.Slice {
		return &EthereumGasFundingCreateBulk{err: fmt.Errorf("calling to EthereumGasFundingClient.MapCreateBulk with wrong type %T, need slice", slice)}
	}
	builders := make([]*EthereumGasFundingCreate, rv.Len())
	for i := 0; i < rv.Len(); i++ {
		builders[i] = c.Create()
		setFunc(builders[i], i)
	}
...
// Update returns an update builder for EthereumGasFunding.
func (c *EthereumGasFundingClient) Update() *EthereumGasFundingUpdate {
	mutation := newEthereumGasFundingMutation(c.config, OpUpdate)
	return &EthereumGasFundingUpdate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOne returns an update builder for the given entity.
func (c *EthereumGasFundingClient) UpdateOne(_m *EthereumGasFunding) *EthereumGasFundingUpdateOne {
	mutation := newEthereumGasFundingMutation(c.config, OpUpdateOne, withEthereumGasFunding(_m))
	return &EthereumGasFundingUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOneID returns an update builder for the given id.
func (c *EthereumGasFundingClient) UpdateOneID(id int64) *EthereumGasFundingUpdateOne {
	mutation := newEthereumGasFundingMutation(c.config, OpUpdateOne, withEthereumGasFundingID(id))
	return &EthereumGasFundingUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// Delete returns a delete builder for EthereumGasFunding.
func (c *EthereumGasFundingClient) Delete() *EthereumGasFundingDelete {
	mutation := newEthereumGasFundingMutation(c.config, OpDelete)
	return &EthereumGasFundingDelete{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// DeleteOne returns a builder for deleting the given entity.
func (c *EthereumGasFundingClient) DeleteOne(_m *EthereumGasFunding) *EthereumGasFundingDeleteOne {
	return c.DeleteOneID(_m.ID)
}

// DeleteOneID returns a builder for deleting the given entity by its id.
func (c *EthereumGasFundingClient) DeleteOneID(id int64) *EthereumGasFundingDeleteOne {
	builder := c.Delete().Where(ethereumgasfunding.ID(id))
	builder.mutation.id = &id
	builder.mutation.op = OpDeleteOne
	return &EthereumGasFundingDeleteOne{builder}
}

// Query returns a query builder for EthereumGasFunding.
func (c *EthereumGasFundingClient) Query() *EthereumGasFundingQuery {
	return &EthereumGasFundingQuery{
		config: c.config,
		ctx:    &QueryContext{Type: TypeEthereumGasFunding},
		inters: c.Interceptors(),
	}
}

// Get returns a EthereumGasFunding entity by its id.
func (c *EthereumGasFundingClient) Get(ctx context.Context, id int64) (*EthereumGasFunding, error) {
	return c.Query().Where(ethereumgasfunding.ID(id)).Only(ctx)
}

// GetX is like Get, but panics if an error occurs.
func (c *EthereumGasFundingClient) GetX(ctx context.Context, id int64) *EthereumGasFunding {
	obj, err := c.Get(ctx, id)
	if err != nil {
		panic(err)
	}
...
// QueryIntent queries the intent edge of a EthereumGasFunding.
func (c *EthereumGasFundingClient) QueryIntent(_m *EthereumGasFunding) *OnchainPaymentIntentQuery {
	query := (&OnchainPaymentIntentClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := _m.ID
		step := sqlgraph.NewStep(
			sqlgraph.From(ethereumgasfunding.Table, ethereumgasfunding.FieldID, id),
...
// Hooks returns the client hooks.
func (c *EthereumGasFundingClient) Hooks() []Hook {
	return c.hooks.EthereumGasFunding
}

// Interceptors returns the client interceptors.
func (c *EthereumGasFundingClient) Interceptors() []Interceptor {
	return c.inters.EthereumGasFunding
}

func (c *EthereumGasFundingClient) mutate(ctx context.Context, m *EthereumGasFundingMutation) (Value, error) {
	switch m.Op() {
	case OpCreate:
		return (&EthereumGasFundingCreate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdate:
		return (&EthereumGasFundingUpdate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdateOne:
		return (&EthereumGasFundingUpdateOne{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpDelete, OpDeleteOne:
		return (&EthereumGasFundingDelete{config: c.config, hooks: c.Hooks(), mutation: m}).Exec(ctx)
	default:
		return nil, fmt.Errorf("ent: unknown EthereumGasFunding mutation op: %q", m.Op())
	}
}

// EthereumNonceStateClient is a client for the EthereumNonceState schema.
type EthereumNonceStateClient struct {
	config
}

// NewEthereumNonceStateClient returns a client for the EthereumNonceState from the given config.
func NewEthereumNonceStateClient(c config) *EthereumNonceStateClient {
	return &EthereumNonceStateClient{config: c}
}

// Use adds a list of mutation hooks to the hooks stack.
// A call to `Use(f, g, h)` equals to `ethereumnoncestate.Hooks(f(g(h())))`.
func (c *EthereumNonceStateClient) Use(hooks ...Hook) {
	c.hooks.EthereumNonceState = append(c.hooks.EthereumNonceState, hooks...)
}

// Intercept adds a list of query interceptors to the interceptors stack.
// A call to `Intercept(f, g, h)` equals to `ethereumnoncestate.Intercept(f(g(h())))`.
func (c *EthereumNonceStateClient) Intercept(interceptors ...Interceptor) {
	c.inters.EthereumNonceState = append(c.inters.EthereumNonceState, interceptors...)
}

// Create returns a builder for creating a EthereumNonceState entity.
func (c *EthereumNonceStateClient) Create() *EthereumNonceStateCreate {
	mutation := newEthereumNonceStateMutation(c.config, OpCreate)
	return &EthereumNonceStateCreate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// CreateBulk returns a builder for creating a bulk of EthereumNonceState entities.
func (c *EthereumNonceStateClient) CreateBulk(builders ...*EthereumNonceStateCreate) *EthereumNonceStateCreateBulk {
	return &EthereumNonceStateCreateBulk{config: c.config, builders: builders}
}

// MapCreateBulk creates a bulk creation builder from the given slice. For each item in the slice, the function creates
// a builder and applies setFunc on it.
func (c *EthereumNonceStateClient) MapCreateBulk(slice any, setFunc func(*EthereumNonceStateCreate, int)) *EthereumNonceStateCreateBulk {
	rv := reflect.ValueOf(slice)
	if rv.Kind() != reflect.Slice {
		return &EthereumNonceStateCreateBulk{err: fmt.Errorf("calling to EthereumNonceStateClient.MapCreateBulk with wrong type %T, need slice", slice)}
	}
	builders := make([]*EthereumNonceStateCreate, rv.Len())
	for i := 0; i < rv.Len(); i++ {
		builders[i] = c.Create()
		setFunc(builders[i], i)
	}
...
// Update returns an update builder for EthereumNonceState.
func (c *EthereumNonceStateClient) Update() *EthereumNonceStateUpdate {
	mutation := newEthereumNonceStateMutation(c.config, OpUpdate)
	return &EthereumNonceStateUpdate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOne returns an update builder for the given entity.
func (c *EthereumNonceStateClient) UpdateOne(_m *EthereumNonceState) *EthereumNonceStateUpdateOne {
	mutation := newEthereumNonceStateMutation(c.config, OpUpdateOne, withEthereumNonceState(_m))
	return &EthereumNonceStateUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOneID returns an update builder for the given id.
func (c *EthereumNonceStateClient) UpdateOneID(id int64) *EthereumNonceStateUpdateOne {
	mutation := newEthereumNonceStateMutation(c.config, OpUpdateOne, withEthereumNonceStateID(id))
	return &EthereumNonceStateUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// Delete returns a delete builder for EthereumNonceState.
func (c *EthereumNonceStateClient) Delete() *EthereumNonceStateDelete {
	mutation := newEthereumNonceStateMutation(c.config, OpDelete)
	return &EthereumNonceStateDelete{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// DeleteOne returns a builder for deleting the given entity.
func (c *EthereumNonceStateClient) DeleteOne(_m *EthereumNonceState) *EthereumNonceStateDeleteOne {
	return c.DeleteOneID(_m.ID)
}

// DeleteOneID returns a builder for deleting the given entity by its id.
func (c *EthereumNonceStateClient) DeleteOneID(id int64) *EthereumNonceStateDeleteOne {
	builder := c.Delete().Where(ethereumnoncestate.ID(id))
	builder.mutation.id = &id
	builder.mutation.op = OpDeleteOne
	return &EthereumNonceStateDeleteOne{builder}
}

// Query returns a query builder for EthereumNonceState.
func (c *EthereumNonceStateClient) Query() *EthereumNonceStateQuery {
	return &EthereumNonceStateQuery{
		config: c.config,
		ctx:    &QueryContext{Type: TypeEthereumNonceState},
		inters: c.Interceptors(),
	}
}

// Get returns a EthereumNonceState entity by its id.
func (c *EthereumNonceStateClient) Get(ctx context.Context, id int64) (*EthereumNonceState, error) {
	return c.Query().Where(ethereumnoncestate.ID(id)).Only(ctx)
}

// GetX is like Get, but panics if an error occurs.
func (c *EthereumNonceStateClient) GetX(ctx context.Context, id int64) *EthereumNonceState {
	obj, err := c.Get(ctx, id)
	if err != nil {
		panic(err)
	}
...
// Hooks returns the client hooks.
func (c *EthereumNonceStateClient) Hooks() []Hook {
	return c.hooks.EthereumNonceState
}

// Interceptors returns the client interceptors.
func (c *EthereumNonceStateClient) Interceptors() []Interceptor {
	return c.inters.EthereumNonceState
}

func (c *EthereumNonceStateClient) mutate(ctx context.Context, m *EthereumNonceStateMutation) (Value, error) {
	switch m.Op() {
	case OpCreate:
		return (&EthereumNonceStateCreate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdate:
		return (&EthereumNonceStateUpdate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdateOne:
		return (&EthereumNonceStateUpdateOne{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpDelete, OpDeleteOne:
		return (&EthereumNonceStateDelete{config: c.config, hooks: c.Hooks(), mutation: m}).Exec(ctx)
	default:
		return nil, fmt.Errorf("ent: unknown EthereumNonceState mutation op: %q", m.Op())
	}
}
...
// QueryIntent queries the intent edge of a OnchainDeposit.
func (c *OnchainDepositClient) QueryIntent(_m *OnchainDeposit) *OnchainPaymentIntentQuery {
	query := (&OnchainPaymentIntentClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := _m.ID
		step := sqlgraph.NewStep(
			sqlgraph.From(onchaindeposit.Table, onchaindeposit.FieldID, id),
			sqlgraph.To(onchainpaymentintent.Table, onchainpaymentintent.FieldID),
			sqlgraph.Edge(sqlgraph.M2O, true, onchaindeposit.IntentTable, onchaindeposit.IntentColumn),
		)
		fromV = sqlgraph.Neighbors(_m.driver.Dialect(), step)
		return fromV, nil
	}
...
// OnchainPaymentIntentClient is a client for the OnchainPaymentIntent schema.
type OnchainPaymentIntentClient struct {
	config
}

// NewOnchainPaymentIntentClient returns a client for the OnchainPaymentIntent from the given config.
func NewOnchainPaymentIntentClient(c config) *OnchainPaymentIntentClient {
	return &OnchainPaymentIntentClient{config: c}
}

// Use adds a list of mutation hooks to the hooks stack.
// A call to `Use(f, g, h)` equals to `onchainpaymentintent.Hooks(f(g(h())))`.
func (c *OnchainPaymentIntentClient) Use(hooks ...Hook) {
	c.hooks.OnchainPaymentIntent = append(c.hooks.OnchainPaymentIntent, hooks...)
}

// Intercept adds a list of query interceptors to the interceptors stack.
// A call to `Intercept(f, g, h)` equals to `onchainpaymentintent.Intercept(f(g(h())))`.
func (c *OnchainPaymentIntentClient) Intercept(interceptors ...Interceptor) {
	c.inters.OnchainPaymentIntent = append(c.inters.OnchainPaymentIntent, interceptors...)
}

// Create returns a builder for creating a OnchainPaymentIntent entity.
func (c *OnchainPaymentIntentClient) Create() *OnchainPaymentIntentCreate {
	mutation := newOnchainPaymentIntentMutation(c.config, OpCreate)
	return &OnchainPaymentIntentCreate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// CreateBulk returns a builder for creating a bulk of OnchainPaymentIntent entities.
func (c *OnchainPaymentIntentClient) CreateBulk(builders ...*OnchainPaymentIntentCreate) *OnchainPaymentIntentCreateBulk {
	return &OnchainPaymentIntentCreateBulk{config: c.config, builders: builders}
}

// MapCreateBulk creates a bulk creation builder from the given slice. For each item in the slice, the function creates
// a builder and applies setFunc on it.
func (c *OnchainPaymentIntentClient) MapCreateBulk(slice any, setFunc func(*OnchainPaymentIntentCreate, int)) *OnchainPaymentIntentCreateBulk {
	rv := reflect.ValueOf(slice)
	if rv.Kind() != reflect.Slice {
		return &OnchainPaymentIntentCreateBulk{err: fmt.Errorf("calling to OnchainPaymentIntentClient.MapCreateBulk with wrong type %T, need slice", slice)}
	}
	builders := make([]*OnchainPaymentIntentCreate, rv.Len())
	for i := 0; i < rv.Len(); i++ {
		builders[i] = c.Create()
		setFunc(builders[i], i)
	}
...
// Update returns an update builder for OnchainPaymentIntent.
func (c *OnchainPaymentIntentClient) Update() *OnchainPaymentIntentUpdate {
	mutation := newOnchainPaymentIntentMutation(c.config, OpUpdate)
	return &OnchainPaymentIntentUpdate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOne returns an update builder for the given entity.
func (c *OnchainPaymentIntentClient) UpdateOne(_m *OnchainPaymentIntent) *OnchainPaymentIntentUpdateOne {
	mutation := newOnchainPaymentIntentMutation(c.config, OpUpdateOne, withOnchainPaymentIntent(_m))
	return &OnchainPaymentIntentUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOneID returns an update builder for the given id.
func (c *OnchainPaymentIntentClient) UpdateOneID(id int64) *OnchainPaymentIntentUpdateOne {
	mutation := newOnchainPaymentIntentMutation(c.config, OpUpdateOne, withOnchainPaymentIntentID(id))
	return &OnchainPaymentIntentUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// Delete returns a delete builder for OnchainPaymentIntent.
func (c *OnchainPaymentIntentClient) Delete() *OnchainPaymentIntentDelete {
	mutation := newOnchainPaymentIntentMutation(c.config, OpDelete)
	return &OnchainPaymentIntentDelete{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// DeleteOne returns a builder for deleting the given entity.
func (c *OnchainPaymentIntentClient) DeleteOne(_m *OnchainPaymentIntent) *OnchainPaymentIntentDeleteOne {
	return c.DeleteOneID(_m.ID)
}

// DeleteOneID returns a builder for deleting the given entity by its id.
func (c *OnchainPaymentIntentClient) DeleteOneID(id int64) *OnchainPaymentIntentDeleteOne {
	builder := c.Delete().Where(onchainpaymentintent.ID(id))
	builder.mutation.id = &id
	builder.mutation.op = OpDeleteOne
	return &OnchainPaymentIntentDeleteOne{builder}
}

// Query returns a query builder for OnchainPaymentIntent.
func (c *OnchainPaymentIntentClient) Query() *OnchainPaymentIntentQuery {
	return &OnchainPaymentIntentQuery{
		config: c.config,
		ctx:    &QueryContext{Type: TypeOnchainPaymentIntent},
		inters: c.Interceptors(),
	}
}

// Get returns a OnchainPaymentIntent entity by its id.
func (c *OnchainPaymentIntentClient) Get(ctx context.Context, id int64) (*OnchainPaymentIntent, error) {
	return c.Query().Where(onchainpaymentintent.ID(id)).Only(ctx)
}

// GetX is like Get, but panics if an error occurs.
func (c *OnchainPaymentIntentClient) GetX(ctx context.Context, id int64) *OnchainPaymentIntent {
	obj, err := c.Get(ctx, id)
	if err != nil {
		panic(err)
	}
...
// QueryPaymentOrder queries the payment_order edge of a OnchainPaymentIntent.
func (c *OnchainPaymentIntentClient) QueryPaymentOrder(_m *OnchainPaymentIntent) *PaymentOrderQuery {
	query := (&PaymentOrderClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := _m.ID
		step := sqlgraph.NewStep(
			sqlgraph.From(onchainpaymentintent.Table, onchainpaymentintent.FieldID, id),
...
// QueryDeposits queries the deposits edge of a OnchainPaymentIntent.
func (c *OnchainPaymentIntentClient) QueryDeposits(_m *OnchainPaymentIntent) *OnchainDepositQuery {
	query := (&OnchainDepositClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := _m.ID
		step := sqlgraph.NewStep(
			sqlgraph.From(onchainpaymentintent.Table, onchainpaymentintent.FieldID, id),
...
// QueryWalletSweeps queries the wallet_sweeps edge of a OnchainPaymentIntent.
func (c *OnchainPaymentIntentClient) QueryWalletSweeps(_m *OnchainPaymentIntent) *WalletSweepQuery {
	query := (&WalletSweepClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := _m.ID
		step := sqlgraph.NewStep(
			sqlgraph.From(onchainpaymentintent.Table, onchainpaymentintent.FieldID, id),
...
// QueryEthereumGasFundings queries the ethereum_gas_fundings edge of a OnchainPaymentIntent.
func (c *OnchainPaymentIntentClient) QueryEthereumGasFundings(_m *OnchainPaymentIntent) *EthereumGasFundingQuery {
	query := (&EthereumGasFundingClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := _m.ID
		step := sqlgraph.NewStep(
			sqlgraph.From(onchainpaymentintent.Table, onchainpaymentintent.FieldID, id),
...
// Hooks returns the client hooks.
func (c *OnchainPaymentIntentClient) Hooks() []Hook {
	return c.hooks.OnchainPaymentIntent
}

// Interceptors returns the client interceptors.
func (c *OnchainPaymentIntentClient) Interceptors() []Interceptor {
	return c.inters.OnchainPaymentIntent
}

func (c *OnchainPaymentIntentClient) mutate(ctx context.Context, m *OnchainPaymentIntentMutation) (Value, error) {
	switch m.Op() {
	case OpCreate:
		return (&OnchainPaymentIntentCreate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdate:
		return (&OnchainPaymentIntentUpdate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdateOne:
		return (&OnchainPaymentIntentUpdateOne{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpDelete, OpDeleteOne:
		return (&OnchainPaymentIntentDelete{config: c.config, hooks: c.Hooks(), mutation: m}).Exec(ctx)
	default:
		return nil, fmt.Errorf("ent: unknown OnchainPaymentIntent mutation op: %q", m.Op())
	}
}
...
// QueryOnchainPaymentIntent queries the onchain_payment_intent edge of a PaymentOrder.
func (c *PaymentOrderClient) QueryOnchainPaymentIntent(_m *PaymentOrder) *OnchainPaymentIntentQuery {
	query := (&OnchainPaymentIntentClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := _m.ID
		step := sqlgraph.NewStep(
			sqlgraph.From(paymentorder.Table, paymentorder.FieldID, id),
			sqlgraph.To(onchainpaymentintent.Table, onchainpaymentintent.FieldID),
			sqlgraph.Edge(sqlgraph.O2O, false, paymentorder.OnchainPaymentIntentTable, paymentorder.OnchainPaymentIntentColumn),
		)
		fromV = sqlgraph.Neighbors(_m.driver.Dialect(), step)
		return fromV, nil
	}
...
// WalletDerivationCursorClient is a client for the WalletDerivationCursor schema.
type WalletDerivationCursorClient struct {
	config
}

// NewWalletDerivationCursorClient returns a client for the WalletDerivationCursor from the given config.
func NewWalletDerivationCursorClient(c config) *WalletDerivationCursorClient {
	return &WalletDerivationCursorClient{config: c}
}

// Use adds a list of mutation hooks to the hooks stack.
// A call to `Use(f, g, h)` equals to `walletderivationcursor.Hooks(f(g(h())))`.
func (c *WalletDerivationCursorClient) Use(hooks ...Hook) {
	c.hooks.WalletDerivationCursor = append(c.hooks.WalletDerivationCursor, hooks...)
}

// Intercept adds a list of query interceptors to the interceptors stack.
// A call to `Intercept(f, g, h)` equals to `walletderivationcursor.Intercept(f(g(h())))`.
func (c *WalletDerivationCursorClient) Intercept(interceptors ...Interceptor) {
	c.inters.WalletDerivationCursor = append(c.inters.WalletDerivationCursor, interceptors...)
}

// Create returns a builder for creating a WalletDerivationCursor entity.
func (c *WalletDerivationCursorClient) Create() *WalletDerivationCursorCreate {
	mutation := newWalletDerivationCursorMutation(c.config, OpCreate)
	return &WalletDerivationCursorCreate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// CreateBulk returns a builder for creating a bulk of WalletDerivationCursor entities.
func (c *WalletDerivationCursorClient) CreateBulk(builders ...*WalletDerivationCursorCreate) *WalletDerivationCursorCreateBulk {
	return &WalletDerivationCursorCreateBulk{config: c.config, builders: builders}
}

// MapCreateBulk creates a bulk creation builder from the given slice. For each item in the slice, the function creates
// a builder and applies setFunc on it.
func (c *WalletDerivationCursorClient) MapCreateBulk(slice any, setFunc func(*WalletDerivationCursorCreate, int)) *WalletDerivationCursorCreateBulk {
	rv := reflect.ValueOf(slice)
	if rv.Kind() != reflect.Slice {
		return &WalletDerivationCursorCreateBulk{err: fmt.Errorf("calling to WalletDerivationCursorClient.MapCreateBulk with wrong type %T, need slice", slice)}
	}
	builders := make([]*WalletDerivationCursorCreate, rv.Len())
	for i := 0; i < rv.Len(); i++ {
		builders[i] = c.Create()
		setFunc(builders[i], i)
	}
...
// Update returns an update builder for WalletDerivationCursor.
func (c *WalletDerivationCursorClient) Update() *WalletDerivationCursorUpdate {
	mutation := newWalletDerivationCursorMutation(c.config, OpUpdate)
	return &WalletDerivationCursorUpdate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOne returns an update builder for the given entity.
func (c *WalletDerivationCursorClient) UpdateOne(_m *WalletDerivationCursor) *WalletDerivationCursorUpdateOne {
	mutation := newWalletDerivationCursorMutation(c.config, OpUpdateOne, withWalletDerivationCursor(_m))
	return &WalletDerivationCursorUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOneID returns an update builder for the given id.
func (c *WalletDerivationCursorClient) UpdateOneID(id int64) *WalletDerivationCursorUpdateOne {
	mutation := newWalletDerivationCursorMutation(c.config, OpUpdateOne, withWalletDerivationCursorID(id))
	return &WalletDerivationCursorUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// Delete returns a delete builder for WalletDerivationCursor.
func (c *WalletDerivationCursorClient) Delete() *WalletDerivationCursorDelete {
	mutation := newWalletDerivationCursorMutation(c.config, OpDelete)
	return &WalletDerivationCursorDelete{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// DeleteOne returns a builder for deleting the given entity.
func (c *WalletDerivationCursorClient) DeleteOne(_m *WalletDerivationCursor) *WalletDerivationCursorDeleteOne {
	return c.DeleteOneID(_m.ID)
}

// DeleteOneID returns a builder for deleting the given entity by its id.
func (c *WalletDerivationCursorClient) DeleteOneID(id int64) *WalletDerivationCursorDeleteOne {
	builder := c.Delete().Where(walletderivationcursor.ID(id))
	builder.mutation.id = &id
	builder.mutation.op = OpDeleteOne
	return &WalletDerivationCursorDeleteOne{builder}
}

// Query returns a query builder for WalletDerivationCursor.
func (c *WalletDerivationCursorClient) Query() *WalletDerivationCursorQuery {
	return &WalletDerivationCursorQuery{
		config: c.config,
		ctx:    &QueryContext{Type: TypeWalletDerivationCursor},
		inters: c.Interceptors(),
	}
}

// Get returns a WalletDerivationCursor entity by its id.
func (c *WalletDerivationCursorClient) Get(ctx context.Context, id int64) (*WalletDerivationCursor, error) {
	return c.Query().Where(walletderivationcursor.ID(id)).Only(ctx)
}

// GetX is like Get, but panics if an error occurs.
func (c *WalletDerivationCursorClient) GetX(ctx context.Context, id int64) *WalletDerivationCursor {
	obj, err := c.Get(ctx, id)
	if err != nil {
		panic(err)
	}
...
// Hooks returns the client hooks.
func (c *WalletDerivationCursorClient) Hooks() []Hook {
	return c.hooks.WalletDerivationCursor
}

// Interceptors returns the client interceptors.
func (c *WalletDerivationCursorClient) Interceptors() []Interceptor {
	return c.inters.WalletDerivationCursor
}

func (c *WalletDerivationCursorClient) mutate(ctx context.Context, m *WalletDerivationCursorMutation) (Value, error) {
	switch m.Op() {
	case OpCreate:
		return (&WalletDerivationCursorCreate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdate:
		return (&WalletDerivationCursorUpdate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdateOne:
		return (&WalletDerivationCursorUpdateOne{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpDelete, OpDeleteOne:
		return (&WalletDerivationCursorDelete{config: c.config, hooks: c.Hooks(), mutation: m}).Exec(ctx)
	default:
		return nil, fmt.Errorf("ent: unknown WalletDerivationCursor mutation op: %q", m.Op())
	}
}

// WalletSweepClient is a client for the WalletSweep schema.
type WalletSweepClient struct {
	config
}

// NewWalletSweepClient returns a client for the WalletSweep from the given config.
func NewWalletSweepClient(c config) *WalletSweepClient {
	return &WalletSweepClient{config: c}
}

// Use adds a list of mutation hooks to the hooks stack.
// A call to `Use(f, g, h)` equals to `walletsweep.Hooks(f(g(h())))`.
func (c *WalletSweepClient) Use(hooks ...Hook) {
	c.hooks.WalletSweep = append(c.hooks.WalletSweep, hooks...)
}

// Intercept adds a list of query interceptors to the interceptors stack.
// A call to `Intercept(f, g, h)` equals to `walletsweep.Intercept(f(g(h())))`.
func (c *WalletSweepClient) Intercept(interceptors ...Interceptor) {
	c.inters.WalletSweep = append(c.inters.WalletSweep, interceptors...)
}

// Create returns a builder for creating a WalletSweep entity.
func (c *WalletSweepClient) Create() *WalletSweepCreate {
	mutation := newWalletSweepMutation(c.config, OpCreate)
	return &WalletSweepCreate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// CreateBulk returns a builder for creating a bulk of WalletSweep entities.
func (c *WalletSweepClient) CreateBulk(builders ...*WalletSweepCreate) *WalletSweepCreateBulk {
	return &WalletSweepCreateBulk{config: c.config, builders: builders}
}

// MapCreateBulk creates a bulk creation builder from the given slice. For each item in the slice, the function creates
// a builder and applies setFunc on it.
func (c *WalletSweepClient) MapCreateBulk(slice any, setFunc func(*WalletSweepCreate, int)) *WalletSweepCreateBulk {
	rv := reflect.ValueOf(slice)
	if rv.Kind() != reflect.Slice {
		return &WalletSweepCreateBulk{err: fmt.Errorf("calling to WalletSweepClient.MapCreateBulk with wrong type %T, need slice", slice)}
	}
	builders := make([]*WalletSweepCreate, rv.Len())
	for i := 0; i < rv.Len(); i++ {
		builders[i] = c.Create()
		setFunc(builders[i], i)
	}
...
// Update returns an update builder for WalletSweep.
func (c *WalletSweepClient) Update() *WalletSweepUpdate {
	mutation := newWalletSweepMutation(c.config, OpUpdate)
	return &WalletSweepUpdate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOne returns an update builder for the given entity.
func (c *WalletSweepClient) UpdateOne(_m *WalletSweep) *WalletSweepUpdateOne {
	mutation := newWalletSweepMutation(c.config, OpUpdateOne, withWalletSweep(_m))
	return &WalletSweepUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOneID returns an update builder for the given id.
func (c *WalletSweepClient) UpdateOneID(id int64) *WalletSweepUpdateOne {
	mutation := newWalletSweepMutation(c.config, OpUpdateOne, withWalletSweepID(id))
	return &WalletSweepUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// Delete returns a delete builder for WalletSweep.
func (c *WalletSweepClient) Delete() *WalletSweepDelete {
	mutation := newWalletSweepMutation(c.config, OpDelete)
	return &WalletSweepDelete{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// DeleteOne returns a builder for deleting the given entity.
func (c *WalletSweepClient) DeleteOne(_m *WalletSweep) *WalletSweepDeleteOne {
	return c.DeleteOneID(_m.ID)
}

// DeleteOneID returns a builder for deleting the given entity by its id.
func (c *WalletSweepClient) DeleteOneID(id int64) *WalletSweepDeleteOne {
	builder := c.Delete().Where(walletsweep.ID(id))
	builder.mutation.id = &id
	builder.mutation.op = OpDeleteOne
	return &WalletSweepDeleteOne{builder}
}

// Query returns a query builder for WalletSweep.
func (c *WalletSweepClient) Query() *WalletSweepQuery {
	return &WalletSweepQuery{
		config: c.config,
		ctx:    &QueryContext{Type: TypeWalletSweep},
		inters: c.Interceptors(),
	}
}

// Get returns a WalletSweep entity by its id.
func (c *WalletSweepClient) Get(ctx context.Context, id int64) (*WalletSweep, error) {
	return c.Query().Where(walletsweep.ID(id)).Only(ctx)
}

// GetX is like Get, but panics if an error occurs.
func (c *WalletSweepClient) GetX(ctx context.Context, id int64) *WalletSweep {
	obj, err := c.Get(ctx, id)
	if err != nil {
		panic(err)
	}
...
// QueryIntent queries the intent edge of a WalletSweep.
func (c *WalletSweepClient) QueryIntent(_m *WalletSweep) *OnchainPaymentIntentQuery {
	query := (&OnchainPaymentIntentClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := _m.ID
		step := sqlgraph.NewStep(
			sqlgraph.From(walletsweep.Table, walletsweep.FieldID, id),
			sqlgraph.To(onchainpaymentintent.Table, onchainpaymentintent.FieldID),
...
// Hooks returns the client hooks.
func (c *WalletSweepClient) Hooks() []Hook {
	return c.hooks.WalletSweep
}

// Interceptors returns the client interceptors.
func (c *WalletSweepClient) Interceptors() []Interceptor {
	return c.inters.WalletSweep
}

func (c *WalletSweepClient) mutate(ctx context.Context, m *WalletSweepMutation) (Value, error) {
	switch m.Op() {
	case OpCreate:
		return (&WalletSweepCreate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdate:
		return (&WalletSweepUpdate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdateOne:
		return (&WalletSweepUpdateOne{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpDelete, OpDeleteOne:
		return (&WalletSweepDelete{config: c.config, hooks: c.Hooks(), mutation: m}).Exec(ctx)
	default:
		return nil, fmt.Errorf("ent: unknown WalletSweep mutation op: %q", m.Op())
	}
}
...
type (
	hooks struct {
		...
		ChainScanCursor, ChannelMonitor, ChannelMonitorDailyRollup,
		ChannelMonitorHistory, ChannelMonitorRequestTemplate, CompositeModelRoute,
		ErrorPassthroughRule, EthereumGasFunding, EthereumNonceState, Group,
		IdempotencyRecord, IdentityAdoptionDecision, OnchainDeposit,
		OnchainPaymentIntent, PaymentAuditLog, PaymentOrder, PaymentProviderInstance,
		...
		WalletDerivationCursor, WalletSweep []ent.Hook
	}
	inters struct {
		APIKey, Account, AccountGroup, Announcement, AnnouncementRead, AuthIdentity,
		AuthIdentityChannel, BatchImageEvent, BatchImageItem, BatchImageJob,
		...
		UsageCleanupTask, UsageLog, User, UserAllowedGroup, UserAttributeDefinition,
		UserAttributeValue, UserPlatformQuota, UserSubscription,
		WalletDerivationCursor, WalletSweep []ent.Interceptor
	}
)
