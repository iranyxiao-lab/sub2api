...
func (tx *Tx) init() {
	...
	tx.ChainScanCursor = NewChainScanCursorClient(tx.config)
	...
	tx.EthereumGasFunding = NewEthereumGasFundingClient(tx.config)
	tx.EthereumNonceState = NewEthereumNonceStateClient(tx.config)
	tx.Group = NewGroupClient(tx.config)
	tx.IdempotencyRecord = NewIdempotencyRecordClient(tx.config)
	tx.IdentityAdoptionDecision = NewIdentityAdoptionDecisionClient(tx.config)
	tx.OnchainDeposit = NewOnchainDepositClient(tx.config)
	tx.OnchainPaymentIntent = NewOnchainPaymentIntentClient(tx.config)
	...
	tx.WalletDerivationCursor = NewWalletDerivationCursorClient(tx.config)
	tx.WalletSweep = NewWalletSweepClient(tx.config)
}
