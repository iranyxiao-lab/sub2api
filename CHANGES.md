...
- 智力测试默认执行上限由 120 秒调整为 300 秒，支持配置 30–1800 秒；每次运行保存时限快照，执行租约与计划锁同步计算，旧记录保持 120 秒。
- 结果卡片显示实际时限，超时继续保留部分回答；新增首/末 SSE 事件耗时及完成状态诊断，不自动重试。
- 合入工作区已有的单题选择修复：立即测试先保存当前选择，未确认的提交保持幂等保护。
- 新增迁移 243，仅增加带默认值的列，兼容旧版本读写。
- homestay 已发布最终镜像 `intelligence-timeout-c4c50767c`；Chrome 两次真实测试成功，最终版本完成 10 分钟健康观察。完整验证范围、既有 Ollama 测试失败及回滚信息见 `docs/intelligence-timeout-release-20260925.zh.md`。
