    ...
    <div class="max-h-[75vh] space-y-4 overflow-y-auto text-sm text-gray-800 dark:text-gray-200">
      <div v-if="!accountId" class="space-y-3">
        ...
        <div v-for="question in questions" :key="question.id" class="flex items-start justify-between gap-3 border-b border-gray-200 py-2 dark:border-dark-600">
          <div class="min-w-0"><p class="font-medium">{{ question.title }}</p><p class="whitespace-pre-wrap break-words text-gray-500">{{ question.prompt }}</p></div>
      ...
      <template v-else>
        ...
        <div v-if="plan" class="border-t border-gray-200 pt-3 dark:border-dark-600">
          ...
          <div v-for="result in results" :key="result.id" class="border-b border-gray-200 py-3 dark:border-dark-600">
            <div class="flex flex-wrap items-center justify-between gap-2"><span class="font-medium">{{ result.question_snapshot?.title || '-' }} · {{ t(result.status === 'failed' ? 'admin.scheduledTests.failed' : 'admin.scheduledTests.success') }} · {{ result.latency_ms }}ms</span><span class="text-xs text-gray-500">{{ formatDateTime(result.started_at) }}</span></div>
            <details class="mt-2"><summary class="cursor-pointer text-primary-600">{{ t('admin.intelligence.details') }}</summary>
              ...
              <button v-if="result.response_text" class="btn btn-secondary my-2" @click="preview = htmlPreview(result.response_text)">{{ t('admin.intelligence.preview') }}</button>
            </details>
          </div>
        </div>
      </template>
...
const editQuestion = (question: IntelligenceQuestion) => { editingQuestion.value = { ...question } }
const saveQuestion = async () => {
  if (!editingQuestion.value) return
  busy.value = true
  try { await adminAPI.scheduledTests.saveQuestion(editingQuestion.value); editingQuestion.value = null; await load() }
