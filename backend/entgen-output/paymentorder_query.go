...
type PaymentOrderQuery struct {
	...
	withUser                 *UserQuery
	withOnchainPaymentIntent *OnchainPaymentIntentQuery
...
// QueryOnchainPaymentIntent chains the current query on the "onchain_payment_intent" edge.
func (_q *PaymentOrderQuery) QueryOnchainPaymentIntent() *OnchainPaymentIntentQuery {
...
func (_q *PaymentOrderQuery) Clone() *PaymentOrderQuery {
	...
	return &PaymentOrderQuery{
		...
		withOnchainPaymentIntent: _q.withOnchainPaymentIntent.Clone(),
...
// WithOnchainPaymentIntent tells the query-builder to eager-load the nodes that are connected to
// the "onchain_payment_intent" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *PaymentOrderQuery) WithOnchainPaymentIntent(opts ...func(*OnchainPaymentIntentQuery)) *PaymentOrderQuery {
	query := (&OnchainPaymentIntentClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withOnchainPaymentIntent = query
	return _q
}
...
func (_q *PaymentOrderQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*PaymentOrder, error) {
	var (
		...
		loadedTypes = [2]bool{
			_q.withUser != nil,
			_q.withOnchainPaymentIntent != nil,
		}
	)
	...
	if query := _q.withOnchainPaymentIntent; query != nil {
		if err := _q.loadOnchainPaymentIntent(ctx, query, nodes, nil,
			func(n *PaymentOrder, e *OnchainPaymentIntent) { n.Edges.OnchainPaymentIntent = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *PaymentOrderQuery) loadOnchainPaymentIntent(ctx context.Context, query *OnchainPaymentIntentQuery, nodes []*PaymentOrder, init func(*PaymentOrder), assign func(*PaymentOrder, *OnchainPaymentIntent)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(onchainpaymentintent.FieldPaymentOrderID)
	}
	query.Where(predicate.OnchainPaymentIntent(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(paymentorder.OnchainPaymentIntentColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
