...
func TestIntelligenceRunDurabilityAndRecovery(t *testing.T) {
	...
	for i := 0; i < 8; i++ {
		...
		go func() {
			defer wg.Done()
			run, err := results.EnqueueIntelligenceRun(ctx, plan.ID, "request-1", "manual", nil)
			if err != nil {
				errs <- err
				return
			}
	...
	other, err := results.EnqueueIntelligenceRun(ctx, plan.ID, "request-2", "manual", nil)
	require.NoError(t, err)
	...
	same, err := results.EnqueueIntelligenceRun(ctx, plan.ID, "request-1", "manual", nil)
	require.NoError(t, err)
	...
	second, err := results.EnqueueIntelligenceRun(ctx, plan.ID, "request-3", "manual", nil)
	require.NoError(t, err)
	...
	third, err := results.EnqueueIntelligenceRun(ctx, plan.ID, "request-4", "manual", nil)
	require.NoError(t, err)
	...
	_, err = results.EnqueueIntelligenceRun(ctx, plan.ID, "schedule-early", "scheduled", &next)
	...
	require.NoError(t, err)
	scheduled, err := results.EnqueueIntelligenceRun(ctx, plan.ID, "schedule-due", "scheduled", &next)
	require.NoError(t, err)
